...
def activities_scraper(soup, browser, athlete_id):
    ...
    try:
        ...
        for month_url in month_url_ls:
            month_soup = make_soup_sel(month_url, browser)
            """
...
#TODO
# Collect follower/following locations
def followings_scraper(browser, athlete_id, type_str='following'):
    ...
    try:
        ...
        soup = make_soup_mech(url, browser)
        # get how many pages
        try:
        ...
        for i in range(1, page_num + 1):
            pagination_url = "https://www.strava.com/athletes/" + str(athlete_id) + "/follows?page=" \
                             + str(i) + "&type=" + type_str
            soup = make_soup_mech(pagination_url, browser)
        ...
        # 	# If only needs athlete ids
        # 	following_ath_id_ls.append(following['data-athlete-id'])
        # 	# If needs both athlete names and ids
        # 	#
        # 	# following_ath_id = following['data-athlete-id']
        # 	# following_ath_name = following.find("div", {"class" : "h4 topless"}).find("a").get_text().encode('utf8')
        # 	# following_ath_id_ls.append({"athlete_id" : following_ath_id, "athlete_name" : following_ath_name})
        # 	#
    except AttributeError:
        pass

    return following_ath_id_ls
...
def main():
    ...
    for user_id in user_ids:
        ...
        soup_mech = make_soup_mech(page, br)
        soup_sel = make_soup_sel(page, browser)
        try:
            alert = soup_mech.find("div", {"class": "messages"}).get_text().replace('\n', '')
        ...
        ath_profile_one, flag = profile_scraper(soup_mech, user_id)
        if flag == True:
            continue
        # 2. Scrape the athlete's social stats
        social_stats_one = soc_stats_scraper(soup_mech)
        # 3. Scrape the athlete's activity stats
        activity_stats_one = act_stats_scraper(soup_sel)
        # 4. Scrape activity entries
        activities_one = activities_scraper(soup_sel, br, user_id)
        # 5. Scrape who the user is following
        following_one = followings_scraper(br, user_id, 'following')
        # 6. Scrape who is following the user
        follower_one = followings_scraper(br, user_id, 'followers')

        if len(following_one) == 0:
